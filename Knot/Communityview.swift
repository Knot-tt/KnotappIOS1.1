...
struct CommunitiesView: View {
        ...
        ZStack(alignment: .bottom) {
            VStack(spacing: 0) {
                ...
                HStack(spacing: 10) {
                    HStack {
                        ...
                        TextField("Search Knots", text: $searchText).autocapitalization(.none)
                    ...
                    Button(action: { showFilterSheet = true }) {
                        Image(systemName: "slider.horizontal.3")
                            .font(.system(size: 18))
                            .foregroundColor(filtersActive ? .white : .black)
                            .padding(10)
                            .background(filtersActive ? Color.black : Color(.systemGray6))
                            .cornerRadius(12)
                    }
                }
                        ...
                        LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 16) {
                            ForEach(displayedGroups) { group in
                                GroupCard(
                                    ...
                                    isAdmin            : group.adminName == profile.name || group.coAdminNames.contains(profile.name),
            ...
            Button(action: { showCreateGroup = true }) {
                Image(systemName: "plus.circle.fill")
                    .font(.system(size: 44)).foregroundColor(.black)
    ...
    private var emptyState: some View {
        VStack(spacing: 16) {
            Spacer()
            Image(systemName: emptyIcon).font(.system(size: 48)).foregroundColor(Color(.systemGray3))
            Text(emptyTitle).font(.headline).foregroundColor(Color(.systemGray))
            Text(emptySubtitle).font(.caption).foregroundColor(Color(.systemGray3))
    ...
    private var eventsPlaceholder: some View {
        VStack(spacing: 16) {
            Spacer()
            Image(systemName: "calendar.badge.clock").font(.system(size: 48)).foregroundColor(Color(.systemGray3))
            Text("Knot Events").font(.headline).foregroundColor(Color(.systemGray))
            Text("Events will appear here once the feature is live.")
                .font(.caption).foregroundColor(Color(.systemGray3)).multilineTextAlignment(.center).padding(.horizontal, 40)
            Spacer()
        }.frame(maxWidth: .infinity)
    }
...
struct GroupCard: View {
    ...
    private let whitePercent: CGFloat = 0.35
    ...
    var body: some View {
        ZStack(alignment: .topLeading) {
            VStack(spacing: 0) {
                ...
                Group {
                    if let urlStr = group.imageURL, let url = URL(string: urlStr) {
                        AsyncImage(url: url) { phase in
                            ...
                            case .failure, .empty:
                                Color(.systemGray5)
                                    .overlay(Image(systemName: group.imageName).font(.system(size: 48)).foregroundColor(.black))
                            @unknown default:
                                Color(.systemGray5)
                            }
                        }
                ...
                .frame(height: cardHeight * (1 - whitePercent))
                .clipped()

                // White info strip — exactly 35% of card height
                VStack(spacing: 2) {
                    HStack(spacing: 4) {
                        Text(group.name).font(.subheadline).fontWeight(.semibold).foregroundColor(.black)
                            .lineLimit(1)
                        if isAdmin {
                    ...
                    Text("by \(group.adminName)").font(.caption2).foregroundColor(Color(.systemGray))
                        .lineLimit(1)
                ...
                .frame(maxWidth: .infinity)
                .frame(height: cardHeight * whitePercent)
            ...
            if group.isEvent {
                HStack {
                    Spacer()
                    Text("Event").font(.caption2).fontWeight(.semibold).foregroundColor(.white)
...
struct KnotDetailView: View {
    ...
    var isCreator   : Bool { liveGroup.adminName == profile.name }
    var isCoAdmin   : Bool { liveGroup.coAdminNames.contains(profile.name) }
    ...
    var body: some View {
        NavigationStack {
                ...
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    ZStack {
                        if let urlStr = liveGroup.imageURL, let url = URL(string: urlStr) {
                            AsyncImage(url: url) { phase in
                                ...
                                case .failure, .empty:
                                    Rectangle().fill(Color(.systemGray5)).frame(height: 220)
                                        .overlay(Image(systemName: liveGroup.imageName).font(.system(size: 80)).foregroundColor(.black))
                                @unknown default:
                                    Rectangle().fill(Color(.systemGray5)).frame(height: 220)
                                }
                            }
                            ...
                            Rectangle().fill(Color(.systemGray5)).frame(height: 220)
                            Image(systemName: liveGroup.imageName).font(.system(size: 80)).foregroundColor(.black)
                        }
                        if liveGroup.isEvent {
                            VStack {
                                HStack {
                                    Spacer()
                                    Text("Knot Event").font(.caption).fontWeight(.semibold).foregroundColor(.white)
                                        .padding(.horizontal, 10).padding(.vertical, 5)
                    ...
                    VStack(alignment: .leading, spacing: 14) {
                        ...
                        // Name
                        Text(liveGroup.name).font(.system(size: 26, weight: .bold)).foregroundColor(.black)
                        ...
                        Button(action: { if !isAdmin { showAdminActions = true } }) {
                            HStack(spacing: 8) {
                                ...
                                VStack(alignment: .leading, spacing: 1) {
                                    Text("Created by").font(.caption).foregroundColor(.gray)
                                    Text(liveGroup.adminName).font(.subheadline).fontWeight(.semibold).foregroundColor(.black)
                                }
                                if !isAdmin {
                        ...
                        FlowLayout(spacing: 8) {
                            ...
                            if liveGroup.isPaid {
                                metaChip(icon: "dollarsign.circle.fill",
                                         text: "$\(liveGroup.price) · \(liveGroup.paymentType.rawValue)",
                                         color: .black)
                            }
                        }
                        ...
                        Text("About this Knot").font(.headline).foregroundColor(.black)
                        Text(liveGroup.description).font(.body).foregroundColor(.black.opacity(0.75)).lineSpacing(5)

                        // ── Join / Status ────────────────────────────────
                        if isAdmin {
                            VStack(spacing: 10) {
                            Label(isCreator ? "You manage this Knot" : "You're an admin", systemImage: "star.fill")
                                .fontWeight(.semibold).foregroundColor(Color(.systemGray))
                            ...
                            Button(action: {
                                Task { await profile.joinKnot(liveGroup) }
                            }) {
                                Text(liveGroup.requiresApproval ? "Request to Join" : "Join Knot")
                                    .fontWeight(.semibold).foregroundColor(.white)
                                    .frame(maxWidth: .infinity).padding()
                                    .background(Color.black).cornerRadius(12)
                            }
                        ...
                        Button(action: {
                            ...
                            DispatchQueue.main.asyncAfter(deadline: .now() + 0.35) {
                                if admin_ {
                                    ...
                                    profile.openConversation(with: admin,
                                                             sourceKnotID: knotID,
                                                             sourceKnotName: knotName)
                                }
                            }
                        }) {
                            Text(isAdmin ? "Open Knot Group Chat" : "Message Knot")
                                .fontWeight(.semibold).foregroundColor(.black)
                                .frame(maxWidth: .infinity).padding()
                                .background(Color.white).cornerRadius(12)
                                .overlay(RoundedRectangle(cornerRadius: 12).stroke(Color.black, lineWidth: 1.5))
            ...
            .confirmationDialog(liveGroup.adminName, isPresented: $showAdminActions, titleVisibility: .visible) {
                Button("Message \(liveGroup.adminName)") { profile.selectedTab = .messages; dismiss() }
                Button("View Profile") { showUserProfile = liveGroup.adminName }
                Button("Add as Connection") { /* TODO: Supabase */ }
                Button("Cancel", role: .cancel) {}
            } message: { Text("What would you like to do?") }
            .sheet(item: $showUserProfile) { name in UserProfileView(name: name).environment(profile) }
...
struct UserProfileView: View {
    ...
    private var privacy      : UserProfile.OtherUserPrivacy { profile.privacy(for: name) }
    private var isConnected  : Bool { profile.connections.contains(name) }
    private var showRestricted: Bool { privacy.isPrivate && !isConnected }
        ...
        NavigationStack {
            ScrollView {
                VStack(spacing: 24) {
                    ...
                    HStack(spacing: 0) {
                        VStack(spacing: 2) {
                            Text("—").font(.system(size: 18, weight: .bold)).foregroundColor(.black)
                            Text("Connections").font(.caption).foregroundColor(.gray)
                        }
                        .frame(maxWidth: .infinity)
                        Divider().frame(height: 32)
                        VStack(spacing: 2) {
                            if privacy.showListings {
                                let count = profile.allListings.filter { $0.sellerName == name }.count
                                Text("\(count)").font(.system(size: 18, weight: .bold)).foregroundColor(.black)
                            } else {
                                Text("—").font(.system(size: 18, weight: .bold)).foregroundColor(.black)
                            }
                            Text("Listings").font(.caption).foregroundColor(.gray)
                        }
                        .frame(maxWidth: .infinity)
                        Divider().frame(height: 32)
                        VStack(spacing: 2) {
                            if privacy.showKnots {
                                Text("\(knots.count)").font(.system(size: 18, weight: .bold)).foregroundColor(.black)
                            } else {
                                Text("—").font(.system(size: 18, weight: .bold)).foregroundColor(.black)
                            }
                            Text("Knots").font(.caption).foregroundColor(.gray)
                        }
                        .frame(maxWidth: .infinity)
                    }
                    .padding(.vertical, 8)
                    ...
                    if showRestricted {
                        ...
                        VStack(spacing: 8) {
                            Image(systemName: "lock.fill").font(.system(size: 32)).foregroundColor(Color(.systemGray3))
                            Text("This account is private")
                                .font(.subheadline).fontWeight(.semibold).foregroundColor(Color(.systemGray))
                            Text("Connect with \(name) to see their Knots and activity.")
                                .font(.caption).foregroundColor(Color(.systemGray3))
                                .multilineTextAlignment(.center).padding(.horizontal, 32)
                            if !profile.sentConnectionRequests.contains(name) {
                                Button(action: {
                                    if let uid = userID {
                        ...
                        if !theirListings.isEmpty && privacy.showListings {
                                ...
                                ForEach(knots) { knot in
                                    HStack(spacing: 12) {
                                        ZStack {
                                            ...
                                            Image(systemName: knot.imageName).font(.system(size: 18)).foregroundColor(.black)
                                        }
                                        VStack(alignment: .leading, spacing: 2) {
                                            Text(knot.name).font(.subheadline).fontWeight(.semibold)
                                            Text(knot.category).font(.caption).foregroundColor(.gray)
                                        }
                                        Spacer()
                                        if knot.adminName == name {
                                            Text("Admin").font(.caption2).foregroundColor(.white)
                                                .padding(.horizontal, 7).padding(.vertical, 3)
                    ...
                    Menu {
                        ...
                        if profile.connections.contains(name) {
                            ...
                            Button(action: {
                                ...
                                Label("Add as Connection", systemImage: "person.badge.plus")
                            }
                        }
                    } label: {
        ...
        NavigationStack {
                            ...
                            ForEach(categories, id: \.self) { cat in
                                ...
                                Button(action: {
                                    ...
                                    Text(cat).font(.subheadline)
                                        ...
                                        .background(sel ? Color.black : Color.white)
                                        .foregroundColor(sel ? .white : .black)
                                        .cornerRadius(20)
                                        .overlay(RoundedRectangle(cornerRadius: 20)
                                            .stroke(sel ? Color.black : Color(.systemGray3), lineWidth: 1))
                                }
                                .buttonStyle(.plain)
                            }
                        }
            ...
            .toolbar {
                ToolbarItem(placement: .cancellationAction) {
                    ...
                    .foregroundColor(noFilters ? .gray : .black)
                    .disabled(noFilters)
                }
...
struct AdminSendAlertView: View {
            ...
            Form {
                ...
                Section {
                    Button(action: sendAlert) {
                        Text("Send to \(group.memberCount) Members")
                            .frame(maxWidth: .infinity, alignment: .center)
                            .fontWeight(.semibold)
                            .foregroundColor(canSend ? .white : .gray)
                    }
                    .listRowBackground(canSend ? Color.black : Color(.systemGray5))
                    .disabled(!canSend)
                }
            }
    ...
    private func sendAlert() {
        ...
        sent = true
        Task {
            await profile.sendAnnouncement(knotID: group.id, title: t, body: b, isPinned: false)
        }
    }
}
                        ...
                        Button(action: { selectedNewCreator = name }) {
                            HStack(spacing: 12) {
                                ...
                                if liveGroup.coAdminNames.contains(name) {
                                    Text("Admin").font(.caption2).foregroundColor(.gray)
                                }
                    ...
                    VStack(spacing: 8) {
                        Button(action: {
                            ...
                            Text("Transfer & Leave")
                                .fontWeight(.semibold).foregroundColor(.white)
                                .frame(maxWidth: .infinity).padding()
                                .background(selectedNewCreator == nil ? Color.gray : Color.black)
                                .cornerRadius(12)
                        }
