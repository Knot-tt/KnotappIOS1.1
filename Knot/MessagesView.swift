            ...
            VStack(spacing: 0) {
                ...
                HStack {
                    Menu {
                        ...
                        HStack(spacing: 6) {
                            Text("Messages")
                                .font(.system(size: 34, weight: .bold)).foregroundColor(.black)
...
struct ChatView: View {
    ...
    private func sendMessage() {
        ...
        // Text: send to DB; message appears when sendMessage() appends the returned row
        if !text.isEmpty {
            Task { await profile.sendMessage(text: text, conversationID: conversationID, replyToID: replyID) }
        }

        // Images: append locally only (Storage upload is a future Phase 4 extension)
        if let ci = convoIndex, !imgs.isEmpty {
            for img in imgs {
                profile.conversations[ci].messages.append(
                    ChatMessage(text: "", image: img, sender: profile.name,
                                timestamp: Date(), status: .sent, replyToID: replyID)
                )
            }
        }
    }
                        ...
                        ForEach(searchResults, id: \.id) { result in
                            Button(action: { selectedPerson = result }) {
                                HStack(spacing: 12) {
                                    ...
                                    VStack(alignment: .leading, spacing: 2) {
                                        Text(result.name).font(.subheadline).fontWeight(.semibold).foregroundColor(.black)
                                        if profile.connections.contains(result.name) {
                                            ...
                                            Text("Neighbour").font(.caption).foregroundColor(.gray)
                                        }
                                    }
                                    Spacer()
                                    if profile.connections.contains(result.name) {
