            ...
            ScrollView {
                VStack(spacing: 20) {
                    ...
                    Button(action: { navigateToPropose = true }) {
                        Label("Accept & Propose Meetup", systemImage: "checkmark.circle.fill")
                            .fontWeight(.semibold)
...
struct SellerProposeMeetupView: View {
    ...
    private func sendProposal() {
        order.meetupProposal = MeetupProposal(
            location: location,
            date: meetupDate,
            proposedBy: "seller"
        )
        ...
        ScrollView {
            VStack(spacing: 28) {
                ...
                VStack(spacing: 6) {
                    ...
                    Text("Complete the exchange then confirm receipt in the app to release payment.")
                        .font(.subheadline)
                        .foregroundColor(.secondary)
                        .multilineTextAlignment(.center)
                }
...
struct OrderTimelineView: View {
    ...
    private func isComplete(_ step: TimelineStep) -> Bool {
        order.stepDates[step.status] != nil || order.status.rawValue == step.status
    }

    private func isCurrent(_ step: TimelineStep) -> Bool {
        order.status.rawValue == step.status && order.stepDates[step.status] == nil
    }
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 20) {
                ...
                // ── Meetup Details ────────────────────────────────────
                if let p = proposal, order.status == .meetupAgreed || order.status == .awaitingConfirmation || order.status == .complete {
                    SectionCard {
                        VStack(spacing: 0) {
                ...
                // ── Listing Summary ───────────────────────────────────
                ListingMiniCard(listing: order.listing, subtitle: order.id)
                ...
                NavigationLink(
                    destination: BuyerConfirmReceiptView(order: .constant(order)),
                    isActive: $navigateToReceipt
                ) { EmptyView() }

                NavigationLink(
                    destination: TransactionCompleteView(order: order, isSeller: isSeller),
                    isActive: $navigateToComplete
                ) { EmptyView() }
        ...
        .navigationBarTitleDisplayMode(.inline)
        .sheet(isPresented: $showReview)  { LeaveReviewView(order: order, isSeller: isSeller) }
        .sheet(isPresented: $showInvoice) { InvoiceView(order: order) }
    }
    ...
    private var actionButton: some View {
        ...
        case .meetupAgreed:
            Button(action: openMessage) {
                Label(currentActionLabel, systemImage: "message.fill")
    ...
    private func timelineRow(step: TimelineStep, idx: Int) -> some View {
        let done    = order.stepDates[step.status] != nil
        let current = order.status.rawValue == step.status
        ...
        return HStack(alignment: .top, spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                if let date = order.stepDates[step.status] {
        ...
        Group {
            if profile.orders.isEmpty {
                VStack(spacing: 16) {
                    ...
                    Text("When you buy something it will appear here.")
                        .font(.subheadline)
                        .foregroundColor(Color(UIColor.systemGray3))
                        .multilineTextAlignment(.center)
